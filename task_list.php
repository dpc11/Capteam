...
if (isset($_GET['select_st'])) {
	switch($_GET['select_st'])
	{
		...
		case "进行中":$colstatus_Recordset1=1; $_SESSION['ser_status'] = "进行中"; break;
		case "已过期":$colstatus_Recordset1=2; $_SESSION['ser_status'] = "已过期"; break;
		case "被驳回":$colstatus_Recordset1=3; $_SESSION['ser_status'] = "被驳回"; break;
		case "已完成":$colstatus_Recordset1=4; $_SESSION['ser_status'] = "已完成"; break;
		case "已验收":$colstatus_Recordset1=5; $_SESSION['ser_status'] = "已验收"; break;
...
if ($pagetabs <> "etask") {
	...
	if (!empty($_SERVER['QUERY_STRING'])) {
		...
		$newParams = array();
		foreach ($params as $param) {
			if (stristr($param, "pageNum_timeout") == false && stristr($param, "totalRows_timeout") == false) {
				array_push($newParams, $param);
			}	
		}
		if (count($newParams) != 0) {
...
if ($pagetabs <> "etask") {
	...
	if (!empty($_SERVER['QUERY_STRING'])) {
		...
		$newParams = array();
		foreach ($params as $param) {
			if (stristr($param, "pageNum_nearout") == false && 	stristr($param, "totalRows_nearout") == false) {
				array_push($newParams, $param);
			}
		}
		if (count($newParams) != 0) {
			...
			<table  class="table  table-hover " style="border-bottom:1px #ddd solid;">
				<?php do { ?>
					<tr>
						...
						<?php } ?>
						<td class="gray">
						<?php $live_days = (strtotime(date("Y-m-d")) - strtotime($row_nearout['csa_plan_et']))/86400;
						echo $multilingual_outofdate_outofdate.": ".$live_days." ".$multilingual_outofdate_date; ?>
						</td>
					</tr>
						...
						<table border="0">
							<tr>
								...
								<td><?php if ($pageNum_nearout < $totalPages_nearout) { // Show if not last page ?>
									<!--下一页 -->
									<a href="<?php printf("%s?pageNum_nearout=%d%s", $currentPage, min($totalPages_nearout, $pageNum_nearout + 1), $queryString_nearout); ?>"><?php echo $multilingual_global_next; ?></a><?php } // Show if not last page ?></td>
								<td><?php if ($pageNum_nearout < $totalPages_nearout) { // Show if not last page ?>
						 			<!--最后一页 -->
									<a href="<?php printf("%s?pageNum_nearout=%d%s", $currentPage, $totalPages_nearout, $queryString_nearout); ?>"><?php echo $multilingual_global_last; ?></a><?php } // Show if not last page ?></td>
							</tr>
						</table>
					</td>
					...
					<form id="form1" name="myform" method="get" class="taskform form-inline">
						...
						<?php if ($pagetabs == "etask") { echo "style=\"display:none;style=\"width:110px;\""; }else { echo "style=\"width:110px;\"";} ?>>
							...
							<?php do {  ?>
								<option value="<?php echo $row_tkstatus['task_status']; ?>" <?php 
									...
									} ?>><?php echo $row_tkstatus['task_status']?>
								</option>
							<?php }while ($row_tkstatus = mysql_fetch_assoc($tkstatus));
						...
						<?php if ($pagetabs == "mtask") { echo "style=\"display:none;style=\"width:160px;\""; }else { echo "style=\"width:160px;\"";} ?>>
								<option value="%"
									<?php if($_SESSION['ser_status']=="%"){
										echo "selected=\"selected\"";
									} ?>><?php echo $multilingual_taskf_touser; ?>
								</option>
								<!-- 不限执行人 -->
								<?php do {  ?>
						...
						<select  class="form-control " name="create_by" id="create_by" <?php if ($pagetabs == "ctask" || $pagetabs == "etask") { echo "style=\"display:none;style=\"width:160px;\""; }else{ echo "style=\"width:160px;\""; } ?>>
							...
							<?php do {  ?>
								<option value="<?php echo $row_Recordset2['uid']?>"
									...
									?>><?php echo $row_Recordset2['tk_display_name']?>
								</option>
							<?php } while ($row_Recordset2 = mysql_fetch_assoc($Recordset2));
					...
					<thead  class="toptable tasktab_tl" >
						<tr>
							...
							<th width="55%;" class="topic" >	  
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_text&order=
								<?php 
								...
								?>><?php echo $multilingual_default_task_title; ?></a>
							</th>
							<th width="30%" class="topic"><a href="<?php echo $pagenames; ?>?
								<?php echo $current_url; ?>&sort=csa_to_user&order=
								<?php 
								...
								?>><?php echo $multilingual_default_task_to; ?></a>
							</th>
						</tr>
					</thead>
					<tbody >
				...
				<table  border="0" cellspacing="0" cellpadding="0" align="center"  class="maintable tasktab_br" >
					<thead  class="toptable " >
						...
						<tr class="righttable_head" >
							<th rowspan="2"  class="status"><a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_status&order=
								<?php if ( $sortlist <> "csa_status"){
									echo "DESC";
								...
								?>><?php echo $multilingual_default_task_status; ?></a>
							</th>
							<!--优先级-->
							<th rowspan="2" class="attr">
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_priority&order=
								<?php  if ( $sortlist <> "csa_priority"){
									echo "DESC";
								...
								?>><?php echo $multilingual_default_task_priority; ?></a>
							</th>
							<!--工作量-->
							<th rowspan="2" class="planpv">
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_plan_hour&order=
								<?php  if ( $sortlist <> "csa_plan_hour"){
									echo "DESC";
								}else if( $sortlist == "csa_plan_hour" && $orderlist == "DESC"){
									echo "ASC";
								} else {
									echo "DESC";
								} ?>" 
								<?php 
									if($sortlist=="csa_plan_hour" && $orderlist=="ASC"){
										echo "class='sort_asc'";
									} else if ($sortlist=="csa_plan_hour" && $orderlist=="DESC"){
										echo "class='sort_desc'";
									}
								?>><?php echo $multilingual_default_task_planpv; ?></a>
							</th> 
							<!--计划开始时间-->
							<th rowspan="2"  class="time" >
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_plan_st&order=
								<?php if ( $sortlist <> "csa_plan_st"){
									echo "DESC";
								...
								?>><?php echo $multilingual_default_task_planstart; ?></a>
							</th>
							<!--计划完成时间-->
							<th rowspan="2"  class="time"  >
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_plan_et&order=
								<?php if ( $sortlist <> "csa_plan_et"){
									echo "DESC";
								...
								?>><?php echo $multilingual_default_task_planend; ?></a>
							</th>
							<!--所属项目-->
							<th rowspan="2" class="attr"  >
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_project&order=
								<?php if ( $sortlist <> "csa_project"){
									echo "DESC";
								...
								?>><?php echo $multilingual_default_task_project; ?></a>
							</th>
							<!--所属阶段-->
							<th rowspan="2" class="attr" >
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_project_stage&order=<?php if ( $sortlist <> "csa_project_stage"){
									echo "DESC";
								}else if( $sortlist == "csa_project_stage" && $orderlist == "DESC"){
									echo "ASC";
								} else {
									echo "DESC";
								} ?>" 
								<?php 
									if($sortlist=="csa_project_stage" && $orderlist=="ASC"){
										echo "class='sort_asc'";
									} else if ($sortlist=="csa_project_stage" && $orderlist=="DESC"){
										echo "class='sort_desc'";
								} ?>><?php echo $multilingual_default_task_stage; ?></a>
							</th>
							<!--来自-->
							<th rowspan="2" class="attr" >
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_from_user&order=<?php 
									...
									?>><?php echo $multilingual_default_task_from; ?></a>
							</th>
							<!--上次更新时间-->
							<th rowspan="2" class="lasttime">
								<a href="<?php echo $pagenames; ?>?<?php echo $current_url; ?>&sort=csa_last_update&order=<?php 
									...
									?>><?php echo $multilingual_default_task_update_time; ?></a>
							</th>
						</tr>
					</thead>     
					<tbody class="tasktab_t2">
						...
						do { ?>
							<tr  class="<?php if($R_list%2==1){ echo "odd_line"; }else{echo "even_line"; } ?>">
								...
								<td class="week_style_padtd" width="70px" align="center" ><?php echo $row_Recordset1['csa_priority']; ?> </td>
								<td class="week_style_padtd" width="80px" align="center" data-ellipsis="true"data-ellipsis-max-width="95px"> <?php echo $row_Recordset1['csa_plan_hour']; ?>&nbsp; </td>
								<td class="week_style_padtd" width="80px" align="center"> <?php echo $row_Recordset1['csa_plan_st']; ?>&nbsp; </td>  
