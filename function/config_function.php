...
function in_2array($strs, $arr){
   ...
   foreach($arr as $value){
     if(in_array($strs, $value)){
        $exist = 1;
        break;    //Ñ­»·ÅÐ¶Ï×Ö·û´®ÊÇ·ñ´æÔÚÓÚÒ»Î»Êý×é£¬´æÔÚÔòÌø³ö  ·µ»Ø½á¹û
     }
   }
   return $exist;
}
...
{ 
$v = join(",",$v); //½µÎ¬,Ò²¿ÉÒÔÓÃimplode,½«Ò»Î¬Êý×é×ª»»ÎªÓÃ¶ººÅÁ¬½ÓµÄ×Ö·û´® 
$temp[$k] = $v; 
} 
$temp = array_unique($temp); //È¥µôÖØ¸´µÄ×Ö·û´®,Ò²¾ÍÊÇÖØ¸´µÄÒ»Î¬Êý×é 
foreach ($temp as $k => $v) 
{ 
$array=explode(",",$v); //ÔÙ½«²ð¿ªµÄÊý×éÖØÐÂ×é×° 
$temp2[$k]["uid"] =$array[0]; 
