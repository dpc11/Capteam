...
var CalHeatMap = function() {
	...
	this.paint = function(navigationDir) {
		...
		var subDomainSvgGroup = svg.append("svg")
			...
			.attr("id", function(d) {
				id_name="";
				var classname = "graph-subdomain-group";
				var date = new Date(d);
				switch(options.domain) {
					/* falls through */
				case "year":
					classname += "_y_" + date.getFullYear();
					id_name += "" + date.getFullYear();
				case "month":
					classname += "_m_" + (date.getMonth() + 1);
					id_name += "-" + (date.getMonth() + 1);
					/* falls through */
				case "day":
					classname += "_d_" + date.getDate();
...
Legend.prototype.redraw = function(width) {
    ...
    //var class_name=calendar.Legend.getClass(this, (calendar.legendScale === null));
	legendItem
		.enter()
		.append("rect")
...
Legend.prototype.getClass = function(n, withCssClass) {
	...
	index.unshift("");
	class_name=(index.join(" r") + (withCssClass ? index.join(" q"): "")).trim()
	return class_name;
};
