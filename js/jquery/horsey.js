...
function horsey (input, options) {
  ...
  if (Array.isArray(suggestions[0].split('||'))) {
    loaded(suggestions[0].split('||'));
  ...
  function add (suggestion) {
    ...
    function select () {
		set(suggestion.split('=')[0]);
		var splitstr=getText(suggestion.split('=')[1]).split('%');
		document.getElementById('uuid').value=splitstr[0];
		document.getElementById('uuname').value=splitstr[1];
		document.getElementById('uuphone').value=splitstr[2];
		document.getElementById('uuemail').value=splitstr[3];
		hide();
		input.focus();
...
function defaultGetValue (suggestion) {
  return typeof suggestion === 'string' ? suggestion : suggestion.split('=')[0];
}

function defaultGetText (suggestion) {
  return typeof suggestion === 'string' ? suggestion : suggestion.split('=')[1];
}
